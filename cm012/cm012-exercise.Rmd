...
levels(draw)
nlevels(draw)
fct_count(draw) # Nice, tibble with # of counts of each factor
```
...
```{r, fig.width=2, fig.height=1}
gapminder %>%
  mutate(continent = fct_reorder(continent, medLifeExp)) %>% 
  ggplot(aes(continent, lifeExp)) +
  geom_violin()
gapminder %>%
  mutate(medLifeExp = median(lifeExp), continent = fct_reorder(continent, medLifeExp)) %>% 
  ggplot(aes(continent, lifeExp)) +
  geom_boxplot()
```
