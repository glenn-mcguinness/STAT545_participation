...
gapminder %>%
  group_by(country) %>%
  summarize(countryMu = mean(lifeExp),
            countryMd = median(lifeExp))

gapminder %>%
  group_by(country, continent) %>%
  summarize(countryContMu = mean(lifeExp),
            countryContMd = median(lifeExp))
```
...
gapminder %>%
  group_by(country) %>%
  mutate(growth = pop - first(pop, order_by = year))
```
...
gapminder %>%
  group_by(country) %>%
  mutate(pop_last_time = pop -  lag(pop))
```
