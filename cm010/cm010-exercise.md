...
``` r
# Can match up columns with different names in this way.
# Specifies col-name from the left and col-name from the right
...
``` r
left_join(a, c) # Joins on all matching colnames unless otherwise specified
```
...
We can't apply set functions between the two datasets because they do not share the same columns names, which is required for set functions.

We can apply binding functions to the two datasets, because sharing the same column names is not required for binding functions.
...
``` r
p = tibble(x1 = c(1,1,2), x2 = 1:3)
q = tibble(x1 = 1:3, x3 = 7:9)
