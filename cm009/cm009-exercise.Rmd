...
```{r}
tibble(let = letters, num = 1:length(letters), paste(letters, num))
```
...
```{r}
write_delim(iris, "irisDollar.csv", delim = "$")
```

Read the dollar-delimited `iris` data to a tibble.

```{r}
read_delim(iris, "irisDollar.csv", delim = "$")
```
...
- https://raw.github.com/jennybc/lotr-tidy/blob/master/data/The_Return_Of_The_King.csv
...
```{r}
lotr_untidy2 = do.call(bind_rows, list(lotr1, lotr2, lotr3))
gather(lotr_untidy2, key = "Gender", value = "Word", Female, Male)
```
...
```{r}
lotr_tidy %>%
  spread(key = "Race", val = "Words")
lotr_tidy %>%
  spread(key = "Gender", val = "Words")
```
...
```{r}
lotr_tidy %>%
  unite(RaceGender, c(Race, Gender)) %>%
  spread(key = RaceGender, val = Words)
lotr_tidy %>%
